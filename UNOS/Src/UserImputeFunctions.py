...
def GaussianMixtureBinning(data, columList, seed, n_init=10):
    """
...
def imputeGaussianMixture(dataSeries, seed, n_components):
    """
    Imputes missing values in a data series using a Gaussian Mixture Model (GMM).
    """
    # apply Min-Max Scaling
    scaler = MinMaxScaler()
    data_scaled = pd.DataFrame(scaler.fit_transform(dataSeries), columns=dataSeries.columns)    
...
def imputeKNN(dataSeries, n_neighbors=range(1, 10), cv=5, figsize=(8,6), flag=True):
    """
